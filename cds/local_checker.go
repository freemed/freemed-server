...
// interactionKey returns a canonical key for a pair of drug classes (alphabetically sorted).
...
var interactionMap = map[string]interactionRule{
	...
	// QT prolongation
	interactionKey("Fluoroquinolone antibiotic", "Macrolide antibiotic"): {
	...
	interactionKey("Beta blocker", "Calcium channel blocker"): {
		SevMajor, "Additive negative inotropic and chronotropic effects. Risk of bradycardia, heart block, and heart failure.",
	},

	// ACE inhibitor + ARB
	interactionKey("ACE inhibitor", "ARB"): {
		SevMajor, "Dual RAAS blockade increases risk of hypotension, syncope, hyperkalemia, and renal impairment. Generally avoid.",
	},
}
...
// getInteraction returns an InteractionResult if drugA and drugB have a known class interaction.
...
func (lc *LocalChecker) CheckDrugAllergy(drug string, allergies []string) ([]InteractionResult, error) {
	...
	for _, allergy := range allergies {
		...
		// Check direct allergy-to-drug-class match via allergy substance map
		...
		// Also check if the allergy text itself directly names a drug class
...
func checkAllergyDrugMatch(drugClass, allergyClass, drugName, allergy string) *InteractionResult {
	...
	// Cross-reactivity: penicillin allergy → cephalosporin caution
