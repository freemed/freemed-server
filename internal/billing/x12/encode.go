...
func Encode837Professional(claim *Claim837P) ([]byte, error) {
	...
	var buf TransactionSetBuffer

	// ISA13/IEA02 require a 9-digit interchange control number; ST02/SE02 use a
	// 4-digit transaction set control number; GS06/GE02 use the group control
	// number. Keeping these distinct is required by X12 005010.
	isaCtrl := fmt.Sprintf("%09d", time.Now().UnixNano()%1000000000)
	...
	isa := NewSegment("ISA",
		...
		RepetitionSep, // ISA11 repetition separator
		"00501",       // ISA12 version
		isaCtrl,       // ISA13 control number (9 digits)
	...
	// Start counting segments for SE01 (ST through SE inclusive).
	buf.CountSegments()
	...
	// ---- SE ----
	// SE01 = number of segments in the transaction set, ST through SE inclusive:
	// segments already written plus this SE segment itself.
	se := NewSegment("SE", intToStr(buf.SegmentCount()+1), tsn)
