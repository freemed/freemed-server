...
-- name: SchedulerDailyApptRangePaginated :many
SELECT s.caldateof AS date_of
, DATE_FORMAT(s.caldateof, '%m/%d/%Y') AS date_of_mdy
, s.calhour AS hour
, s.calminute AS minute
...
-- name: CountSchedulerDailyApptRange :one
...
-- name: SchedulerDailyApptRangeByProviderPaginated :many
SELECT s.caldateof AS date_of
, DATE_FORMAT(s.caldateof, '%m/%d/%Y') AS date_of_mdy
, s.calhour AS hour
, s.calminute AS minute
...
-- name: CountSchedulerDailyApptRangeByProvider :one
SELECT COUNT(DISTINCT s.id) AS total
FROM scheduler s
WHERE ( s.caldateof >= sqlc.arg(from_date) AND s.caldateof <= sqlc.arg(to_date) )
AND s.calstatus NOT IN ( 'noshow', 'cancelled' )
...
-- name: SchedulerFindDateApptPaginated :many
SELECT * FROM scheduler
WHERE caldateof = sqlc.arg(req_date)
AND calstatus != 'cancelled'
LIMIT ? OFFSET ?;

-- name: CountSchedulerDateAppt :one
...
-- name: SchedulerFindDateApptByProviderPaginated :many
SELECT * FROM scheduler
WHERE caldateof = sqlc.arg(req_date)
AND calstatus != 'cancelled'
AND calphysician = sqlc.arg(provider_id)
...
-- name: CountSchedulerDateApptByProvider :one
SELECT COUNT(*) AS total FROM scheduler
WHERE caldateof = sqlc.arg(req_date)
AND calstatus != 'cancelled'
AND calphysician = sqlc.arg(provider_id);
